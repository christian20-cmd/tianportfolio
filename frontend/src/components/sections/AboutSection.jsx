...
function SkillIcon({ icon: Icon, label, color, level, isSvg }) {
  ...
  return (
    ...
    >
      ...
      <div
        ref={bubbleRef}
        className="pointer-events-none absolute -top-16 left-1/2 z-30 w-32 -translate-x-1/2 rounded-lg bg-black px-3 py-2 text-center shadow-lg"
      >
...
function ServiceCard({ Icon, label, sub, stars = 5 }) {
  return (
    <div className="group flex flex-col items-center">
      ...
      <h3 className="font-poppins text-xl font-bold uppercase text-black">{label}</h3>
      <p className="font-poppins text-sm text-start leading-relaxed text-black/60">{sub}</p>
    </div>
  );
}
    ...
    >
      <div className="reveal items-center text-center justify-center flex flex-col w-full pb-8 px-4">
        <div className="flex items-center justify-center gap-2 sm:gap-3 mb-4 w-full max-w-xl">
          ...
          <span className=" text-2xl font-bold text-black">{t.sectionTitle}</span>
          <div className="bar-anim section-bar bg-green-600"></div>
        </div>
      </div>
